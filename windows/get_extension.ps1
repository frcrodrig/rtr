            ...
            foreach ($Fold in (Get-ChildItem $Path|?{$_.Name -ne 'Temp'})) {
                foreach ($Item in (Get-ChildItem -Path $Fold.FullName)) {
                    $Json=Join-Path -Path $Item.FullName -ChildPath manifest.json;if (Test-Path -Path $Json) {
                        ...
                        $Msg=Join-Path -Path (Join-Path -Path $Item.Fullname -ChildPath $_) -ChildPath (
                        messages.json);if(Test-Path -Path $Msg){$App=Get-Content $Msg|ConvertFrom-Json;(@(
                        'appName','extName','extensionName','app_name','application_title',$Id).foreach{if(
                        $App.$_.message){$App.$_.message}})|select -First 1}}};Version=$_.Version;Id=$Fold.Name}}
                    }
                }
            }
        }
