...
}else{[PSCustomObject]@{LastUsedUsername=$Last}|%{@('ActiveUserDomain','ActiveUserLogonServer','ActiveUsername',
    'ActiveUserProfile').foreach{$_.PSObject.Properties.Add((New-Object PSNoteProperty($_,$null)))};$_}
}
$Out=[PSCustomObject]@{Host=[System.Net.Dns]::GetHostname();Script='get_current_user.ps1';Message='check_humio'}
if(gcm shumio -EA 0){shumio $Obj;$Out|%{$_|ConvertTo-Json -Compress}
}else{
